...
<template>
  <div class="list__content" v-if="usersStore.getUser">
    <ol class="list__list">
      <li class="list__item">
        <span class="form__label list__label">Nom : </span>
        <span class="list__text">{{ usersStore.getUser.lastname }}</span>
      </li>
      <li class="list__item">
        <span class="form__label list__label">Prénom : </span>
        <span class="list__text">{{ usersStore.getUser.firstname }}</span>
      </li>
      <li class="list__item">
        <span class="form__label list__label">Email : </span>
        <span class="list__text">{{ usersStore.getUser.email }}</span>
      </li>
      <li class="list__item">
        <span class="form__label list__label">Rôle : </span>
        <span class="list__text">{{
          usersStore.getUser.role === 'user' ? 'Utilisateur' : 'Administrateur'
        }}</span>
      </li>
      <li class="list__item">
        <span class="form__label list__label">Compte activé : </span>
        <span class="list__text">{{ usersStore.getUser.active ? 'Oui' : 'Non' }}</span>
      </li>
      <li class="list__item">
        <span class="form__label list__label">Compte Bloqué : </span>
        <span class="list__text">{{ usersStore.getUser.accountLockedExpire ? 'oui' : 'Non' }}</span>
      </li>
    </ol>
    <div class="list__btn">
      <button
        @click="
          !appStore.getModal
            ? appStore.updateModal({
                ...
                id: { idUser: usersStore.getUser._id }
              })
            : appStore.getModal && appStore.getModal.type === 'adminDeleteSelectedUser'
            ? appStore.resetModal
            : ''
        "
        type="button"
