...
const onSubmit = handleSubmit(
  async (
    ...
    if (route.name === 'resetEmail') {
      await userStore.fetchResetEmail(values as confirmResetEmailInterface, token as string)
    } else if (route.name === 'activationAccount') {
      await userStore.fetchActivationAccountForm(values as loginInterface, token as string)
    } else if (route.name === 'resetPassword') {
      await userStore.fetchResetPassword(values as confirmResetPasswordInterface, token as string)
    } else if (route.name === 'confirmRenewal') {
      await userStore.fetchConfirmRenewalApiKey(values as loginInterface, token as string)
    }
